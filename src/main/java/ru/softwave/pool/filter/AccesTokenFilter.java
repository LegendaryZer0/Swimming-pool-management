package ru.softwave.pool.filter;
...
import ru.softwave.pool.service.TokenAuthenticateService;
...
public class AccesTokenFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        log.info("result is {}",(request.getRequestURI().contains("/v2/api-docs") || (request.getRequestURI().contains("/authenticate"))));
        return (request.getRequestURI().contains("/v2/api-docs")
                || (request.getRequestURI().contains("/authenticate"))
                || (request.getRequestURI().contains("/api"))
                || (request.getRequestURI().contains("/swagger-ui"))
                || (request.getRequestURI().contains("open-api-docs"))
        ); ///open-api-docs
    }
}
